...
public class TransactionService {
	...
	public static Duration getPreviousPeriod(Duration duration) {
		...
		long daysBetween = duration.getLength();
		...
		if (duration.getLength() == -2) {
			// need to change length if its
			LocalDateTime startDate = LocalDateTime.parse(duration.getStartDate(), formatter);
			LocalDateTime endDate = LocalDateTime.parse(duration.getEndDate(), formatter);
			daysBetween = ChronoUnit.DAYS.between(startDate, endDate);
		}
 
		LocalDateTime endPreviousPeriod = now.minus(daysBetween, ChronoUnit.DAYS);
		LocalDateTime startPreviousPeriod = endPreviousPeriod.minus(daysBetween, ChronoUnit.DAYS);
		previousPeriod.setStartDate(startPreviousPeriod.format(formatter));
		previousPeriod.setEndDate(endPreviousPeriod.format(formatter));
		System.out.println("Given Duration of length: " + duration.getLength() + "with start/end: "
				+ duration.getStartDate() + "/" + duration.getEndDate());
		System.out.println("The previous period should be: " + previousPeriod.getLength() + "with start/end: "
